  ...
  <div>
    <el-col :span="24" class="breadcrumb-container">
      <el-button-group style="float: left; margin:10px">
        <el-button type="primary" size="small" @click="addRow(accesscontextdata)">新增</el-button>
      ...
      <el-table-column prop="Active" label="ACTIVE" :formatter="formatter">
        <template slot-scope="scope">
          <el-select v-model="scope.row.Active" clearable>
            <el-option
              v-for="item in accesscontextactivedata"
              :key="item.ID"
              :label="item.Name"
              :value="item.Value">
            </el-option>
  ...
  export default { // 这里需要将模块引出，可在其他地方使用
    name: 'accesscontext',
      ...
      methods:{
        ...
        addRow(accesscontextdata,event){
        ...
        handleSubmit(index, row) {
          console.log(row);
              axios({
                method: "POST",//请求方式
                url: "http://127.0.0.1:8081/v1/admin/flowaccesscontext",//请求地址
                params:{
                  name:row.Name,
                },
                // data: {
